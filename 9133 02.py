...
#question 3
#The new ID
def get_ID(str_new_id):
    length = len(str_new_id)
    if length == 10:
        return str_new_id
    else:
        temp_digit = int(str_new_id[length -1]) + int(str_new_id[length - 2])
        temp_digit = temp_digit % 10
        return get_ID(str_new_id + str(temp_digit))
...
def main():
    ...
    encrypted_name = encrypt_word(dict_letters, full_name)
    print(encrypted_name, shift)
